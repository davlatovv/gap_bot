...
async def start(message: Message, state: FSMContext):
    ...
    if user is not None and user.subscribe == 0:
        ...
        await message.answer(("""🇷🇺🙂Добро пожаловать в “mates"!
😉С нами "Гяп" стала безопаснее и проще!
...
async def authorization_phone(message: Message, state: FSMContext):
    ...
    await message.answer_document(open("data/ПОЛЬЗОВАТЕЛЬСКОЕ СОГЛАШЕНИЕ MATES.docx", 'rb'), reply_markup=contact_keyboard)
